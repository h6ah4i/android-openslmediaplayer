...
#ifndef OPENSLMEDIAPLAYEREQUALIZER_HPP_
#define OPENSLMEDIAPLAYEREQUALIZER_HPP_
...
class OpenSLMediaPlayerEqualizer : public virtual android::RefBase {
public:
    enum { MAX_NUM_BANDS = 32 };

    struct Settings {
        uint16_t curPreset;
        uint16_t numBands;
        int16_t bandLevels[MAX_NUM_BANDS];
    };

    OpenSLMediaPlayerEqualizer(const android::sp<OpenSLMediaPlayerContext> &context) OSLMP_API_ABI;
    virtual ~OpenSLMediaPlayerEqualizer() OSLMP_API_ABI;
...
#endif // OPENSLMEDIAPLAYEREQUALIZER_HPP_
