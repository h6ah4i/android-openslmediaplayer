...
#ifndef VISUALIZERCAPTUREDAUDIODATABUFFER_HPP_
#define VISUALIZERCAPTUREDAUDIODATABUFFER_HPP_
...
class VisualizerCapturedAudioDataBuffer {
public:
    VisualizerCapturedAudioDataBuffer(uint32_t buffer_size_in_frames, uint32_t max_read_size_in_frames);
    ~VisualizerCapturedAudioDataBuffer();

    void reset() noexcept;

    bool put_captured_data(uint32_t num_channels, uint32_t sampling_rate, const int16_t *data, uint32_t num_frames,
    ...
    bool get_latest_captured_data(uint32_t num_frames, uint32_t *num_channels, uint32_t *sampling_rate,
                                  const int16_t **data, timespec *updated_time) noexcept;
...
private:
    ...
    void copy_to_buffer(uint32_t num_channels, const int16_t *src, uint32_t src_pos, uint32_t dest_pos,
                        uint32_t n) noexcept;
...
private:
    ...
    cxxporthelper::aligned_memory<int16_t> buffer_;
...
#endif // VISUALIZERCAPTUREDAUDIODATABUFFER_HPP_
