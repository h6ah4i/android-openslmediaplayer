...
OpenSLMediaPlayerVisualizerJNIBinder::OpenSLMediaPlayerVisualizerJNIBinder(JNIEnv *env, jclass clazz, jobject weak_thiz)
      ...
      methodIdRaiseCaptureEventFromNative_(0), visualizer_(), waveform_buffer_index_(0), fft_buffer_index_(0)
{
    ...
    methodIdRaiseCaptureEventFromNative_ =
        env->GetStaticMethodID(clazz, "raiseCaptureEventFromNative", "(Ljava/lang/Object;I[BI)V");
...
OpenSLMediaPlayerVisualizerJNIBinder::~OpenSLMediaPlayerVisualizerJNIBinder()
{
    JNIEnv *env = nullptr;
...
int OpenSLMediaPlayerVisualizerJNIBinder::bind(const android::sp<OpenSLMediaPlayerVisualizer> &visualizer,
...
void OpenSLMediaPlayerVisualizerJNIBinder::onEnterInternalPeriodicCaptureThread(
    OpenSLMediaPlayerVisualizer *visualizer) noexcept
{
...
void OpenSLMediaPlayerVisualizerJNIBinder::onLeaveInternalPeriodicCaptureThread(
    OpenSLMediaPlayerVisualizer *visualizer) noexcept
{
...
void OpenSLMediaPlayerVisualizerJNIBinder::onAllocateCaptureBuffer(OpenSLMediaPlayerVisualizer *visualizer,
                                                                   int32_t type, int32_t size) noexcept
{
    ...
    case CAPTURE_BUFFER_TYPE_WAVEFORM:
        ...
        for (int i = 0; i < NUM_BUFFERS; ++i) {
            jwaveform_data_[i].assign(env_, env_->NewByteArray(size), jref_type::global_reference);
        }
        break;
    case CAPTURE_BUFFER_TYPE_FFT:
        fft_buffer_index_ = 0;
        for (int i = 0; i < NUM_BUFFERS; ++i) {
            jfft_data_[i].assign(env_, env_->NewByteArray(size), jref_type::global_reference);
        }
        break;
    }
}

void OpenSLMediaPlayerVisualizerJNIBinder::onDeAllocateCaptureBuffer(OpenSLMediaPlayerVisualizer *visualizer,
...
void *OpenSLMediaPlayerVisualizerJNIBinder::onLockCaptureBuffer(OpenSLMediaPlayerVisualizer *visualizer,
                                                                int32_t type) noexcept
{
    if (!jvm_attached_)
        return nullptr;
...
void OpenSLMediaPlayerVisualizerJNIBinder::onUnlockCaptureBuffer(OpenSLMediaPlayerVisualizer *visualizer, int32_t type,
...
void OpenSLMediaPlayerVisualizerJNIBinder::onWaveFormDataCapture(OpenSLMediaPlayerVisualizer *visualizer,
                                                                 const uint8_t *waveform, size_t size,
                                                                 uint32_t samplingRate) noexcept
{
    ...
    raiseCaptureEvent(EVENT_TYPE_ON_WAVEFORM_DATA_CAPTURE, jwaveform_data_[waveform_buffer_index_](), samplingRate);

    // rotate buffer index
    waveform_buffer_index_ = (waveform_buffer_index_ + 1) % NUM_BUFFERS;
}

void OpenSLMediaPlayerVisualizerJNIBinder::onFftDataCapture(OpenSLMediaPlayerVisualizer *visualizer, const int8_t *fft,
                                                            size_t size, uint32_t samplingRate) noexcept
{
    ...
    raiseCaptureEvent(EVENT_TYPE_ON_FFT_DATA_CAPTURE, jfft_data_[fft_buffer_index_](), samplingRate);

    // rotate buffer index
    fft_buffer_index_ = (fft_buffer_index_ + 1) % NUM_BUFFERS;
}

void OpenSLMediaPlayerVisualizerJNIBinder::raiseCaptureEvent(jint type, jbyteArray data, jint samplingRate) noexcept
{
